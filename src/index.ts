...
function resolveBinary(configBinary?: string): string {
  ...
  if (process.env.FRISK_BIN) return process.env.FRISK_BIN;
  return "/usr/local/bin/frisk";
}

type FriskConfig = { binaryPath?: string; allowExternal?: boolean };

/** Check if args require external network access. Throws if not allowed. */
function checkExternalAccess(args: string[], allowExternal: boolean): void {
  const needsNetwork = args.some(a => a === "sync" || a === "--slug" || a === "--remote");
  if (needsNetwork && !allowExternal) {
    throw new Error(
      "Blocked: this operation requires network access (threat intel sync or remote repo cloning). " +
      "Set allowExternal: true in plugin config to enable."
    );
  }
}

async function runFrisk(
  args: string[],
  config?: FriskConfig,
  timeoutMs = 120000
...
export default defineToolPlugin({
  ...
  tools: (tool) => [
    tool({
      ...
      description:
        "Run a security audit on a ClawHub skill slug or local path. Checks malware, dependencies, credential leaks, hidden instructions, and supply-chain risks.",

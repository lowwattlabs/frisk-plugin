...
import { s as upsertAuthProfileWithLock } from "./profiles-CKMjfEZZ.js";
import { n as suggestOAuthProfileIdForLegacyDefault } from "./repair-bNbtRAQi.js";
import "./number-runtime-CORbdVbA.js";
import "./string-coerce-runtime-DzE-BRXv.js";
import { d as resolveClaudeThinkingProfile, f as cloneFirstTemplateModel } from "./provider-model-shared-s9la6RrS.js";
import { n as validateAnthropicSetupToken, t as buildTokenProfileId } from "./provider-auth-token-BA6tpOZK.js";
import { t as applyAuthProfileConfig } from "./provider-auth-helpers-DNygKpBT.js";
import { t as createProviderApiKeyAuthMethod } from "./provider-api-key-auth-7S234FdC.js";
import "./provider-auth-L08Tydtg.js";
import "./cli-runtime-C8RTrvVL.js";
import { o as fetchClaudeUsage } from "./provider-usage-3PcWSP4a.js";
import { n as readClaudeCliCredentialsForSetup, r as readClaudeCliCredentialsForSetupNonInteractive, t as readClaudeCliCredentialsForRuntime } from "./cli-auth-seam-Cx8XNcmW.js";
import { n as CLAUDE_CLI_DEFAULT_ALLOWLIST_REFS, r as CLAUDE_CLI_DEFAULT_MODEL_REF, t as CLAUDE_CLI_BACKEND_ID } from "./cli-constants-C512RtfX.js";
import "./cli-shared-CE4-DWe5.js";
import { t as buildAnthropicCliBackend } from "./cli-backend-CC0vbO5J.js";
import { t as buildClaudeCliCatalogEntries } from "./cli-catalog-C0Gh-TRr.js";
import { t as buildAnthropicCliMigrationResult } from "./cli-migration-CneaSUAJ.js";
import { n as normalizeAnthropicProviderConfigForProvider, t as applyAnthropicConfigDefaults } from "./config-defaults-WEluakfF.js";
import { t as anthropicMediaUnderstandingProvider } from "./media-understanding-provider-BTjSUe0p.js";
import { t as buildReplayPolicy } from "./replay-policy-HKHXunf1.js";
import { l as wrapAnthropicProviderStream } from "./stream-wrappers-D8Bigb7S.js";
...
const ANTHROPIC_OPUS_48_MAX_OUTPUT_TOKENS = 128e3;
...
function buildAnthropicForwardCompatModel(ctx) {
	...
	return {
		id: trimmedModelId,
		name: trimmedModelId,
		provider: normalizeLowercaseStringOrEmpty(ctx.provider) === "claude-cli" ? CLAUDE_CLI_BACKEND_ID : PROVIDER_ID,
		...
		maxTokens: isAnthropicOpus48Model(trimmedModelId) ? ANTHROPIC_OPUS_48_MAX_OUTPUT_TOKENS : 64e3
	};
}
...
function isAnthropicOpus48Model(modelId) {
	const normalized = normalizeLowercaseStringOrEmpty(modelId);
	return [ANTHROPIC_OPUS_48_MODEL_ID, ANTHROPIC_OPUS_48_DOT_MODEL_ID].some((prefix) => normalized.startsWith(prefix));
}
...
function applyAnthropicOpus48MaxTokens(params) {
	if (!isAnthropicOpus48Model(params.modelId)) return;
	if ((params.model.maxTokens ?? 0) >= ANTHROPIC_OPUS_48_MAX_OUTPUT_TOKENS) return;
	return {
		...params.model,
...
function applyAnthropicOpusThinkingLevelMap(params) {
	if (!isAnthropicOpus47OrNewerModel(params.modelId)) return;
	if (params.model.thinkingLevelMap?.xhigh === "xhigh" && params.model.thinkingLevelMap?.max === "max") return;
	return {
		...params.model,
		thinkingLevelMap: {
...
function resolveAnthropicImageMediaInput(modelId, modelName) {
	...
	const largeImageOpus = [modelId, modelName].filter((value) => typeof value === "string").some((ref) => [
		ANTHROPIC_OPUS_48_MODEL_ID,
		ANTHROPIC_OPUS_48_DOT_MODEL_ID,
		ANTHROPIC_OPUS_47_MODEL_ID,
		ANTHROPIC_OPUS_47_DOT_MODEL_ID
...
function normalizeAnthropicResolvedModel(ctx) {
	...
	const outputModel = applyAnthropicOpus48MaxTokens({
		modelId: ctx.modelId,

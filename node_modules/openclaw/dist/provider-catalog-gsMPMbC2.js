...
import { n as buildManifestModelProviderConfig } from "./provider-catalog-shared-D5pfdNmi.js";
...
async function loadNvidiaFeaturedModels() {
	const now = Date.now();
	if (featuredModelCache && isFutureDateTimestampMs(featuredModelCache.expiresAtMs, { nowMs: now })) return featuredModelCache.models;
	featuredModelCache = void 0;
	featuredModelRequest ??= fetchNvidiaFeaturedModels();
	try {
		const models = await featuredModelRequest;
		if (models && models.length > 0) {
			const expiresAtMs = resolveExpiresAtMsFromDurationMs(FEATURED_MODEL_CACHE_TTL_MS, { nowMs: now });
			if (expiresAtMs !== void 0) featuredModelCache = {
				expiresAtMs,
				models
			};
		}

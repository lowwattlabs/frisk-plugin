...
import { t as createWorkboardSqliteStores } from "./sqlite-store-BUDUWgSH.js";
...
var WorkboardStore = class WorkboardStore {
	...
	async updateCard(id, patch, options = {}) {
		...
		const now = Date.now();
		const startedAt = patch.startedAt === void 0 ? status === "running" ? existing.startedAt ?? now : existing.startedAt : normalizeTimestamp(patch.startedAt, 0) || void 0;
		const completedAt = patch.completedAt === void 0 ? status === "done" ? existing.completedAt ?? now : void 0 : normalizeTimestamp(patch.completedAt, 0) || void 0;
		...
		let metadata = normalizeMetadata(patch.metadata, existing.metadata, { allowDependencyLinks: options.allowMetadataDependencyLinks !== false });
		...
		for (const key of [
			...
			"maxRetries",
			"scheduledAt"
		]) if (Object.hasOwn(patch, key) && patch[key] !== void 0) automationPatch[key] = patch[key];
		...
		if (options.enforceStatusHolds && patch.status !== void 0) await this.assertActiveStatusAllowed(existing, next, now);
		if (status !== "done") delete next.completedAt;
		if (patch.startedAt !== void 0 && !startedAt) delete next.startedAt;
		if (patch.completedAt !== void 0 && !completedAt) delete next.completedAt;
		if (metadataIsEmpty(next.metadata)) delete next.metadata;
		...
		return await this.enqueueMutation(async () => {
			...
			for (const card of await this.list()) {
				...
				if (await this.shouldAutoOrchestrate(latest)) {
					const boardId = cardBoardId$1(latest);
					const cap = (await this.boardStore.lookup(boardId))?.board.orchestration?.autoDecomposePerDispatch ?? 3;
...
function selectStartableCards(cards, limit) {
	if (limit <= 0) return [];
	...
	const selected = [];
	for (const card of cards.filter((entry) => entry.status === "ready" && !entry.metadata?.claim && !cardIsArchived(entry)).toSorted(sortReadyCards)) {
		const owner = card.agentId ?? DEFAULT_DISPATCH_OWNER;
...
function registerWorkboardGatewayMethods(params) {
	...
	api.registerGatewayMethod("workboard.cards.dispatch", async ({ respond }) => {
		try {

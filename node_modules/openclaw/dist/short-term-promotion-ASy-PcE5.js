...
import { n as appendMemoryHostEvent } from "./events-6NjQTYm0.js";
import "./memory-host-events-gqTtsF-a.js";
import "./dreaming-shared-D-ppwYmu.js";
import { n as resolveMemoryCoreTimestamp, t as resolveMemoryCoreNowMs } from "./time-Cnl8idsF.js";
...
function normalizeStore(raw, nowIso) {
	...
	if (entriesRaw && typeof entriesRaw === "object") for (const [key, value] of Object.entries(entriesRaw)) {
		...
		const recallDays = Array.isArray(entry.recallDays) ? entry.recallDays.map((valueValue) => normalizeIsoDay(String(valueValue))).filter((valueLocal) => valueLocal !== null) : [];
...
async function readPhaseSignalStore(workspaceDir, nowIso) {
	try {
		...
		if (err instanceof SyntaxError) return emptyPhaseSignalStore(nowIso);
		throw err;
	}
}

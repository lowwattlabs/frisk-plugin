...
import { n as DEFAULT_GATEWAY_PORT } from "./paths-CiHrdfZh.js";
...
import { U as isModelThinkingFormat } from "./zod-schema.core-BedchlTo.js";
import { t as splitTrailingAuthProfile } from "./model-ref-profile-L6rqw6O0.js";
import { i as normalizeStaticProviderModelId } from "./model-ref-shared-Bh0ESsLz.js";
import { r as isKnownCoreToolId } from "./tool-catalog-D5QsrL1H.js";
import { g as resolveToolProfilePolicy, l as mergeAlsoAllowPolicy, p as expandToolGroups } from "./tool-policy-K2Xr4lej.js";
import { n as isToolAllowedByPolicyName } from "./tool-policy-match-B8YPWGDv.js";
import { i as resolveOpenClawMcpTransportAlias, n as isKnownCliMcpTypeAlias } from "./mcp-config-normalize-CEgQZSRA.js";
import { a as resolveGatewayPortWithDefault, i as isGatewayNonLoopbackBindMode, r as hasConfiguredControlUiAllowedOrigins, t as buildDefaultControlUiAllowedOrigins } from "./gateway-control-ui-origins-BezPpmwz.js";
import { n as ensureRecord$2, r as hasOwnKey$1, t as cloneRecord$1 } from "./legacy-config-record-shared-4Hv0ogZ5.js";
	...
	defineLegacyConfigMigration({
		...
		legacyRules: [{
			...
			match: (value) => {
				...
				return providers ? Object.keys(providers).some((providerId) => normalizeProviderId(providerId) === LEGACY_OPENAI_CODEX_PROVIDER_ID) : false;
			}
		}, {
			path: ["models", "providers"],
...
function hasLegacyTtsSpeakerSelectionInAgentLocations(value) {
	...
	if (hasLegacyTtsSpeakerSelection(getRecord(getRecord(agents?.defaults)?.tts))) return true;
...
function hasLegacyTtsSpeakerSelectionInChannelLocations(value) {
	...
	for (const [channelId, channelValue] of Object.entries(channels ?? {})) {
		...
		if (hasLegacyTtsSpeakerSelection(getRecord(channel?.tts))) return true;
		...
		for (const [accountId, accountValue] of Object.entries(accounts ?? {})) {
			...
			if (hasLegacyTtsSpeakerSelection(getRecord(account?.tts)) || hasLegacyTtsSpeakerSelection(getRecord(getRecord(account?.voice)?.tts))) return true;
		}
	}
	return false;
}
...
function hasLegacyTtsEnabledInChannelLocations(value) {
	...
	for (const [channelId, channelValue] of Object.entries(channels ?? {})) {
		...
		for (const [accountId, accountValue] of Object.entries(accounts ?? {})) {
			if (isBlockedObjectKey(accountId)) continue;
			const account = getRecord(accountValue);
			if (hasLegacyTtsEnabled(getRecord(account?.tts)) || hasLegacyTtsEnabled(getRecord(getRecord(account?.voice)?.tts))) return true;
		}
	}
	return false;
}
...
const LEGACY_CONFIG_MIGRATIONS_RUNTIME_TTS = [
	...
	defineLegacyConfigMigration({
		...
		legacyRules: [
			...
			{
				path: ["agents"],
				message: "agents.*.tts speaker selection fields voice/voiceName/voiceId are legacy; use speakerVoice or speakerVoiceId. Run \"openclaw doctor --fix\".",
				match: (value) => hasLegacyTtsSpeakerSelectionInAgentLocations(value)
			},
			{
				path: ["channels"],
				message: "channels.*.tts speaker selection fields voice/voiceName/voiceId are legacy; use speakerVoice or speakerVoiceId. Run \"openclaw doctor --fix\".",
...
//#endregion
export { legacyRuntimeModelAliasRequiresRuntimePolicy as a, mergeMissing as c, migrateLegacyXSearchConfig as i, loadBundledChannelDoctorContractApi as l, LEGACY_CONFIG_MIGRATION_RULES as n, listLegacyRuntimeModelProviderAliases as o, migrateLegacyWebSearchConfig as r, migrateLegacyRuntimeModelRef as s, LEGACY_CONFIG_MIGRATIONS as t };

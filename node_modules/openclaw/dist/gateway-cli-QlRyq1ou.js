...
import { t as addGatewayServiceCommands } from "./register-service-commands-D0X2AxsO.js";
import { n as defaultRuntime } from "./runtime-pp2OBTec.js";
import { t as formatHelpExamples } from "./help-format-CBGqVsX5.js";
import { t as addGatewayRunCommand } from "./run-command-CX9JgI2M.js";
...
const gatewayStatusModuleLoader = createLazyImportLoader(() => import("./gateway-status-Bd3AKvJK.js"));
const gatewayHealthModuleLoader = createLazyImportLoader(() => import("./health-2akb9Aol.js"));
const bonjourDiscoveryModuleLoader = createLazyImportLoader(() => import("./bonjour-discovery-xGZAK5vo.js"));
const wideAreaDnsModuleLoader = createLazyImportLoader(() => import("./widearea-dns-Bufo9fdt.js"));
const healthStyleModuleLoader = createLazyImportLoader(() => import("./terminal-core/health-style.js"));
const usageFormatModuleLoader = createLazyImportLoader(() => import("./usage-format-5MDK4cff.js"));
const stabilityBundleModuleLoader = createLazyImportLoader(() => import("./diagnostic-stability-bundle-D-vC7nb5.js"));
const supportExportModuleLoader = createLazyImportLoader(() => import("./diagnostic-support-export-Cq3MaXIm.js"));
const daemonStatusGatherModuleLoader = createLazyImportLoader(() => import("./status.gather-7o9qlWN1.js"));
...
async function callGatewayCli(method, opts, params) {
	return (await import("./call-BiWr0e1J.js")).callGatewayCli(method, opts, params);
}
async function runGatewayCommand(action, label, opts) {
	try {
		...
		if (opts?.json) {
			const { formatGatewayTransportErrorJson } = await import("./call-Blf_Okhs.js");
...
function registerGatewayCli(program) {
	...
	gatewayCallOpts(gateway.command("stability").description("Fetch payload-free Gateway stability diagnostics").option("--limit <limit>", "Maximum number of recent events", "25").option("--type <type>", "Filter by diagnostic event type").option("--since-seq <seq>", "Only include events after this sequence").option("--bundle [path]", "Read a persisted stability bundle instead of calling Gateway; pass \"latest\" for newest").option("--export", "Write a shareable support diagnostics export", false).option("--output <path>", "Diagnostics export output .zip path").action(async (opts, command) => {
		await runGatewayCommand(async () => {
			const { normalizeDiagnosticStabilityQuery, selectDiagnosticStabilitySnapshot } = await import("./diagnostic-stability-DDLBVJhe.js");
			const rpcOpts = resolveGatewayRpcOptions(opts, command);
	...
	gateway.command("discover").description("Discover gateways via Bonjour (local + wide-area if configured)").option("--timeout <ms>", "Per-command timeout in ms", "2000").option("--json", "Output JSON", false).action(async (opts) => {
		await runGatewayCommand(async () => {
			const [{ readSourceConfigBestEffort }, { discoverGatewayBeacons }, { resolveWideAreaDiscoveryDomain }, { dedupeBeacons, parseDiscoverTimeoutMs, pickBeaconHost, pickGatewayPort, renderBeaconLines }, { withProgress }] = await Promise.all([
				...
				import("./discover-DKlqoK_7.js"),
				import("./progress-nqs7E1d1.js")
			]);

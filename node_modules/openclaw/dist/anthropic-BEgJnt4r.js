import { i as buildBaseOptions, n as transformMessages, o as calculateCost, r as adjustMaxTokensForThinking, s as clampThinkingLevel, t as sanitizeSurrogates } from "./sanitize-unicode-OHmjpZhP.js";
...
function mapThinkingLevelToEffort(model, level) {
	const clampedLevel = level ? clampThinkingLevel(model, level) : void 0;
	const mapped = clampedLevel ? model.thinkingLevelMap?.[clampedLevel] : void 0;
...
function buildParams(model, context, isOAuthTokenResult, options) {
	...
	if (context.tools && context.tools.length > 0) {
		...
		params.tools = convertTools(context.tools, isOAuthTokenResult, compat.supportsEagerToolInputStreaming, compat.supportsCacheControlOnTools ? cacheControl : void 0);
	}
	if (model.reasoning) {
...
function convertMessages(messages, model, isOAuthTokenValue, cacheControl) {
	const params = [];
		...
		if (msg.role === "user") if (typeof msg.content === "string") {
			...
			for (const block of msg.content) if (block.type === "text") {
				...
				if (!block.thinkingSignature || block.thinkingSignature.trim().length === 0) blocks.push({
					...
					const thinking = block.thinkingSignature === "reasoning_content" ? sanitizeSurrogates(block.thinking) : block.thinking;
					blocks.push({
						type: "thinking",
						thinking,

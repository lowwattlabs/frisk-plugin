import { n as isVitestRuntimeEnv } from "./env-5g5TGXM7.js";
import { t as isGatewayModelPricingEnabled } from "./model-pricing-config-CpVOfxdS.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-D3zpWSQQ.js";
import { r as createNoopHeartbeatRunner } from "./server-runtime-startup-services-DCG2gFoz.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-CKpSB3vJ.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-JKxePmUH.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-BCbyqPXQ.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-KIVn7T9o.js");
		if (stopped) return;

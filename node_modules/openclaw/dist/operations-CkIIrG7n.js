...
import { i as buildAgentMainSessionKey, u as normalizeAgentId } from "./session-key-BOpfMTUN.js";
...
const loadOverviewModule = async () => await import("./overview-Q3hHb4LT.js");
const loadModelsSharedModule = async () => await import("./shared-CszViWQT.js");
const loadConfigCliModule = async () => await import("./config-cli-CEda07Kl.js");
const loadDoctorModule = async () => await import("./doctor-CyEF4-wD.js");
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-xPS2B8yx.js");
	if (operation === "start") {
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "plugin-search") {
		...
		await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
			const { runPluginsSearchCommand } = await import("./plugins-search-command-BoGEtzdG.js");
	...
	if (operation.kind === "plugin-uninstall") {
		...
		await (opts.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
			const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-BZ_EjnB5.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-B-ux8Ghk.js")).agentsAddCommand)({
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-sikPI5fI.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-BnwP9c5f.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200

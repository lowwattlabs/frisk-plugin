...
import { c as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-ut_mESRE.js";
...
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-CpoQqPqy.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-DS_cH2ly.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-DJfxb5zp.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-BbMpFGrK.js");
	return modelPickerModulePromise;
}
function loadOnboardConfigModule() {
	onboardConfigModulePromise ??= import("./onboard-config-C6Ox9zlT.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-BPXKFs4T.js"), import("./setup-registry-DHwyPE1H.js")]);
...
async function runSetupWizard(opts, runtimeInput, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-C3pg3YnW.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote---ZO-Uxq.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-oD4ioEgf.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-CVwltfpb.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-QpoJLEPc.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { listChannelPlugins } = await import("./plugins-BbdKFGKi.js");
		const { setupChannels } = await import("./onboard-channels--SrFGnPH.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { setupSearch } = await import("./onboard-search-BZ1jzU-B.js");
	...
	if (opts.skipSkills) await prompter.note(t("wizard.setup.skipSkills"), t("wizard.setup.skillsTitle"));
		...
		const { setupSkills } = await import("./onboard-skills-Ci6Pznrq.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-DNSVvWHw.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-BXwMORYG.js");
	...
	if (!opts.skipHooks) {
		const { setupInternalHooks } = await import("./onboard-hooks-DSHAjyMW.js");
	...
	nextConfig = await writeSetupConfigFile(nextConfig, { allowConfigSizeDrop: configResetPerformed });
	const { finalizeSetupWizard } = await import("./setup.finalize-q4S71vd6.js");

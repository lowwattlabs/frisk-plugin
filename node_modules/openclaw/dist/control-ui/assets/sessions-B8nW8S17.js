import{m as e,p as t}from"./lit-runtime-gkTRPNbC.js";import{o as n,r,t as i}from"./string-coerce-DwnRzrPh.js";import{L as a,U as o,W as s,d as c,f as l,h as u,m as d,nt as f,s as p,u as m,ut as h,v as g}from"./index-DRTyMBOr.js";var _=[`off`,`minimal`,`low`,`medium`,`high`],v=[``,`off`,`on`,`full`],y=[``,`on`,`off`],b=[``,`off`,`on`,`stream`],x=[10,25,50,100];function S(e,t){return Object.hasOwn(e,t)?e[t]??null:null}function C(e,t){return(!e.modelProvider||e.modelProvider===t?.modelProvider)&&(!e.model||e.model===t?.model)}function ee(e,t){let n=C(e,t),r=m(e.thinkingDefault??(n?t?.thinkingDefault:void 0)),i=e.thinkingLevels?.length?e.thinkingLevels:n&&t?.thinkingLevels?.length?t.thinkingLevels:(e.thinkingOptions?.length?e.thinkingOptions:n&&t?.thinkingOptions?.length?t.thinkingOptions:_).map(e=>({id:l(e),label:e}));return[{value:``,label:r},...i.map(e=>({value:l(e.id),label:c(e.id,e.label)}))]}function te(e,t){return!t||e.includes(t)?[...e]:[...e,t]}function w(e,t){return!t||e.some(e=>e.value===t)?[...e]:[...e,{value:t,label:c(t)}]}function ne(){return v.map(e=>({value:e,label:n(e===``?`sessionsView.inherit`:e===`off`?`sessionsView.offExplicit`:`sessionsView.${e}`)}))}function re(){return y.map(e=>({value:e,label:n(e===``?`sessionsView.inherit`:`sessionsView.${e}`)}))}function T(e){switch(e){case`running`:return n(`sessionsView.statusRunning`);case`done`:return n(`sessionsView.statusDone`);case`failed`:return n(`sessionsView.statusFailed`);case`killed`:return n(`sessionsView.statusKilled`);case`timeout`:return n(`sessionsView.statusTimeout`);default:return n(`sessionsView.statusUnknown`)}}function E(e){if(a(e))return{label:n(`sessionsView.statusLive`),tone:`live`};if(e.status===`running`&&e.hasActiveRun===!1)return{label:n(`sessionsView.statusIdle`),tone:`idle`};if(e.status){let t=e.status===`done`?`done`:`failed`;return{label:T(e.status),tone:t}}return e.hasActiveRun===!1?{label:n(`sessionsView.statusIdle`),tone:`idle`}:{label:n(`sessionsView.statusUnknown`),tone:`muted`}}function D(t){let r=E(t),i=`${n(`sessionsView.status`)}: ${r.label}`;return e`
  ...
  `}function H(i,a){let c=i.updatedAt?s(i.updatedAt):n(`common.na`),u=i.thinkingLevel??``,d=u?l(u):``,m=w(ee(i,a.result?.defaults),d),_=i.fastMode===!0?`on`:i.fastMode===!1?`off`:``,v=w(re(),_),y=i.verboseLevel??``,x=w(ne(),y),C=i.reasoningLevel??``,T=te(b,C),E=i.latestCompactionCheckpoint,O=i.compactionCheckpointCount??0,k=Math.max(O,+!!E),A=O>0||!!E,j=a.expandedCheckpointKey===i.key,M=a.checkpointItemsByKey[i.key]??[],I=a.checkpointErrorByKey[i.key],B=`session-checkpoints-${encodeURIComponent(i.key)}`,V=P(k),H=R({row:i,updated:c,checkpointCount:k}),U=r(i.displayName)??null,ae=r(i.label)??``,W=!!(U&&U!==i.key&&U!==ae),G=o(i.key),K=G?S(a.agentIdentityById,G.agentId):null,q=r(K?.emoji)??``,J=r(K?.name)??``,Y=J&&G?`${q?`${q} `:``}${J} (${G.channel})`:null,oe=Y??i.key,X=i.kind!==`global`,Z=a.workboardSessionKeys?.has(i.key)===!0,se=a.workboardBusySessionKey===i.key,ce=X?`${h(`chat`,a.basePath)}?session=${encodeURIComponent(i.key)}`:null,Q=i.kind===`cron`?`data-table-badge--cron`:i.kind===`direct`?`data-table-badge--direct`:i.kind===`group`?`data-table-badge--group`:i.kind===`global`?`data-table-badge--global`:`data-table-badge--unknown`,le=[`session-data-row`,A?`session-data-row--expandable`:``,j?`session-data-row--expanded`:``].filter(Boolean).join(` `),$=()=>{A&&a.onToggleCheckpointDetails(i.key)};return[e`<tr
      class=${le}
      tabindex=${A?`0`:t}
    ...
    >
      ...
      <td>${c}</td>
      <td class="session-token-cell">${p(i)}</td>
      ...
      <td>
        <select
          ...
          @change=${e=>{let t=e.target.value;a.onPatch(i.key,{thinkingLevel:ie(t)})}}
        >
      ...
      <td>
        <select
          ?disabled=${a.loading}
          style="padding: 6px 10px; font-size: 13px; border: 1px solid var(--border); border-radius: var(--radius-sm); min-width: 90px;"
          @change=${e=>{let t=e.target.value;a.onPatch(i.key,{fastMode:t===``?null:t===`on`})}}
        >
...
//# sourceMappingURL=sessions-B8nW8S17.js.map
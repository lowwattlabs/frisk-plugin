...
import { h as shortenHomeInString } from "./utils-CFG_gPqL.js";
...
import { a as formatMissingPluginMessage } from "./error-format-17foa2IM.js";
import { n as collectConfiguredRuntimePluginIds, r as resolveConfiguredRuntimePluginInstallCandidate } from "./configured-runtime-plugin-installs-Bu1WNK6y.js";
//#region src/cli/plugins-cli.runtime.ts
function createModuleLoader(load) {
	let promise;
	return () => promise ??= load();
}
const loadPluginsConfigState = createModuleLoader(() => import("./config-state-CAESzKxm.js"));
const loadPluginsStatus = createModuleLoader(() => import("./status-BsJFhe4I.js"));
const loadPluginsCommandHelpers = createModuleLoader(() => import("./plugins-command-helpers-BX8htIjS.js"));
const loadPluginsRegistryRefresh = createModuleLoader(() => import("./plugins-registry-refresh-BVqTxnxo.js"));
...
async function runPluginsEnableCommand(idInput) {
	...
	const { enablePluginInConfig } = await import("./enable-7VECOy0z.js");
...
async function runPluginsDisableCommand(idInput) {
	...
	const { normalizePluginId } = await loadPluginsConfigState();
	const { buildPluginRegistrySnapshotReport } = await loadPluginsStatus();
	const { setPluginEnabledInConfig } = await import("./plugins-config-DWpqPBV1.js");
	const { refreshPluginRegistryAfterConfigMutation } = await loadPluginsRegistryRefresh();
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsInstallAction(raw, opts) {
	await tracePluginLifecyclePhaseAsync("install command", async () => {
		const { runPluginInstallCommand } = await import("./plugins-install-command-D29PGM5X.js");
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-BQhngi7X.js");
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-C3cGcw4_.js");

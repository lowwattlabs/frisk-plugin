...
import { i as getPluginModuleLoaderStats } from "./plugin-module-loader-cache-DB66BX3_.js";
import { t as STARTUP_UNAVAILABLE_GATEWAY_METHODS } from "./core-descriptors-Co4WOjdH.js";
...
const loadMainSessionRestartRecoveryModule = async () => {
	mainSessionRestartRecoveryModulePromise ??= import("./main-session-restart-recovery-Bto37dH7.js");
...
const loadAgentModelSelectionModule = async () => {
	agentModelSelectionModulePromise ??= import("./model-selection-C6zCbDdO.js");
	return await agentModelSelectionModulePromise;
};
const loadInternalHooksModule = async () => {
	internalHooksModulePromise ??= import("./internal-hooks-B4pmEv1_.js");
	return await internalHooksModulePromise;
};
const loadGatewayRestartSentinelModule = async () => {
	gatewayRestartSentinelModulePromise ??= import("./server-restart-sentinel-BCbyqPXQ.js");
...
function scheduleGatewayMemoryBackend(params) {
	...
	const start = () => {
		import("./server-startup-memory-BJhDKImp.js").then(({ startGatewayMemoryBackend }) => startGatewayMemoryBackend({
			cfg: params.cfg,
			log: params.log
		})).catch((err) => {
...
function scheduleProviderAuthStatePrewarm(params) {
	...
	(async () => {
		const { clearCurrentProviderAuthState, warmCurrentProviderAuthStateOffMainThread } = await import("./model-provider-auth-CT7qBMZC.js");
		const { setAuthProfileFailureHook } = await import("./auth-profiles-DPJLmBnY.js");
...
function scheduleAgentRuntimePluginPrewarm(params) {
	...
	timer = setTimeout(() => {
		...
		measureStartup(params.startupTrace, "post-ready.agent-runtime-plugins", async () => {
			...
			const { ensureRuntimePluginsLoaded } = await import("./runtime-plugins-DvfLgg5B.js");
			const cfg = params.getConfig();
			if (isStopped()) return;
...
function scheduleTranscriptsAutoStartSidecar(params) {
	...
	return schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async (isStopped) => {
			const { createTranscriptsAutoStartService } = await import("./transcripts-tool-D51vQT1a.js");
			if (isStopped()) return;
...
async function waitForAcpRuntimeBackendReady(params) {
	const { getAcpRuntimeBackend } = await import("./registry-vZbicO0g.js");
...
async function prewarmConfiguredPrimaryModel(params) {
	const { resolveAgentModelPrimaryValue } = await import("./model-input-Bx5V0ZIn.js");
	...
	const [{ resolveAgentWorkspaceDir, resolveDefaultAgentDir, resolveDefaultAgentId }, { DEFAULT_MODEL, DEFAULT_PROVIDER }, { isCliProvider, resolveConfiguredModelRef }] = await Promise.all([
		import("./agent-scope-D91MshD8.js"),
	...
	const { ensureOpenClawModelsJson } = await import("./models-config-Q9A9fXSF.js");
...
async function startGatewaySidecars(params) {
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			if (internalHooksConfigured) {
				const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([loadInternalHooksModule(), import("./loader-CGicR9r2.js")]);
	...
	let pluginServices = params.shouldStartPluginServices?.() === false ? null : await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
		try {
			const { startPluginServices } = await import("./services-jg7ygky-.js");
	...
	schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			try {
				const [{ resolveAgentSessionDirs }, { cleanStaleLockFiles }] = await Promise.all([import("./session-dirs-BajHdqpo.js"), import("./session-write-lock-B3bNxepj.js")]);
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-DtARkSWE.js");
	...
	if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped, signal) => {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-wQ4L1kd4.js");
			if (isStopped()) return;
	...
	if (params.cfg.hooks?.gmail?.model) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				loadAgentDefaultsModule(),
				import("./model-catalog-D0ccNE56.js"),
				loadAgentModelSelectionModule()
			]);
			if (isStopped()) return;
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	logGatewayStartup: async (params) => (await import("./server-startup-log-Br6T69hq.js")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-Br0JE8M3.js")).scheduleGatewayUpdateCheck(...args),
	startGatewaySidecars,
	startGatewayTailscaleExposure: async (...args) => (await import("./server-tailscale-XQU8Rm34.js")).startGatewayTailscaleExposure(...args)
};
...
async function startGatewayPostAttachRuntime(params, runtimeDeps = defaultGatewayPostAttachRuntimeDeps) {
	...
	sidecarsPromise.then(async (sidecarsResult) => {
		...
		if (hookRunner?.hasHooks("gateway_start")) {
			const { withPluginHttpRouteRegistry } = await import("./http-registry-CHN04v5u.js");

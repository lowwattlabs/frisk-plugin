...
import { r as hasAmbiguousGatewayAuthModeConfig } from "./auth-mode-policy-CSkl4Db0.js";
...
import { n as maybeArchiveLegacyClawdBrowserProfileResidue, r as noteChromeMcpBrowserReadiness, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-Bn_P7LGD.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-DzTEpXRB.js";
import { a as shellCompletionStatusToRepairEffects, i as shellCompletionStatusToHealthFindings, t as checkShellCompletionStatus } from "./doctor-completion-CsInLrKJ.js";
import { n as disableUnavailableSkillsInConfig } from "./doctor-skills-core-cCtqEFSV.js";
import { i as uiProtocolFreshnessIssueToRepairEffects, r as uiProtocolFreshnessIssueToHealthFinding, t as detectUiProtocolFreshnessIssues } from "./doctor-ui-DlXeuIWQ.js";
import { n as collectDisabledCodexPluginRouteIssues } from "./codex-route-warnings-EukC0pJw.js";
import { t as getSkippedExecRefStaticError } from "./exec-resolution-policy-Dp8Cfnqt.js";
...
const loadDoctorWorkspaceModule = async () => await import("./doctor-workspace-M7Yn-ybk.js");
...
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-D5sp7MhM.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-Dup_jTAf.js");
...
const hooksModelCheck = {
	...
	async detect(ctx) {
		...
		const { loadModelCatalog } = await import("./model-catalog-D0ccNE56.js");
		const { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel } = await import("./model-selection-C6zCbDdO.js");
...
const legacyStateCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-BV_AS8tx.js");
...
const bootstrapSizeCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { buildBootstrapInjectionStats, analyzeBootstrapBudget } = await import("./bootstrap-budget-Clv0DaZs.js");
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-BPbavggI.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-7jPtTrPm.js");
...
const claudeCliCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { noteClaudeCliHealth } = await import("./doctor-claude-cli-BZM8wkZ2.js");
...
const openAIOAuthTlsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { formatOpenAIOAuthTlsPreflightFix, runOpenAIOAuthTlsPreflight, shouldRunOpenAIOAuthTlsPrerequisites } = await import("./oauth-tls-preflight-qkp17Tqp.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	source: "doctor",
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./doctor-cron-CrEyJYf8.js");
...
const gatewayPlatformNotesCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-40ztASUN.js");
